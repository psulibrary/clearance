  ...
  try {
    ...
    const result = await pool.request().query(`
      SELECT TOP 15
        ...
        COUNT(DISTINCT CASE WHEN c.PatronID IS NOT NULL AND c.DateReturned IS NULL AND c.DateWithdrawn IS NULL THEN c.PatronID END) AS activePatrons,
        COUNT(DISTINCT CASE WHEN c.PatronID IS NOT NULL AND c.DateWithdrawn IS NULL THEN c.CopyID END)       AS totalCheckouts,
        COUNT(DISTINCT CASE WHEN c.PatronID IS NOT NULL AND c.DateReturned IS NULL AND c.DateWithdrawn IS NULL AND c.DateDue < GETDATE() THEN c.CopyID END) AS overdueItems
      FROM (
