      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-amber-500 text-white rounded-xl p-5 flex flex-col gap-1">
            ...
            <div className="text-xs text-amber-200">{pct(s?.checkedOut ?? 0, s?.totalItems ?? 0)} utilization rate</div>
          </div>
        ...
        <Section title="Circulation Activity" icon="📤">
          ...
          <Card label="Pending Holds"            value={fmt(s?.pendingHolds)}         sub="waiting for copy" />
          <Card label="Ready for Pickup"         value={fmt(s?.readyHolds)}           sub="holds ready now" color="text-green-700" />
        </Section>

        <Section title="Patron Engagement" icon="👥">
          <Card label="Total Patrons"            value={fmt(s?.totalPatrons)}           />
          <Card label="New Patrons This Year"    value={fmt(s?.newPatronsThisYear)}     color="text-green-700" />
          <Card label="Active Borrowers"         value={fmt(s?.patronsWithCheckouts)}   sub="currently have items out" color="text-blue-700" />
          <Card label="Patrons with Overdue"     value={fmt(s?.patronsWithOverdue)}     sub="need follow-up" color="text-red-700" />
          <Card label="Patron Activation Rate"   value={pct(s?.patronsWithCheckouts ?? 0, s?.totalPatrons ?? 0)} sub="% of patrons actively borrowing" color="text-amber-700" />
        ...
        <Section title="Fines & Revenue" icon="💰">
          <Card label="Outstanding Fines"        value={fmt(s?.activeFines)}           sub="fine records open" color="text-red-700" />
